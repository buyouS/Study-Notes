...
function trackLocations(program) {
    ...
    const stateExpr = create.identifier(globalIds.stateId);
    const doLoops = (node, state, callback) => {
...
function transformImportDeclarations(programs) {
    let moduleCounter = 0;
    let allImports = [];
    for (const program of programs) {
        ...
        for (const node of imports) {
            result = (0, transpiler_1.transformSingleImportDeclaration)(moduleCounter, node).concat(result);
            moduleCounter++;
        }
...
function handleImports(programs) {
    ...
    for (const node of imports) {
        ...
        prefix += `const ${name} = (${moduleText.substring(0, moduleText.length - 1)})(${constants_1.MODULE_PARAMS_ID});\n`;
        moduleCounter++;
        names.push(name);
    }
    return [prefix, names];
