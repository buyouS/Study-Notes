...
exports.createTypeEnvironment = exports.tForAll = exports.tVar = exports.typeCheck = void 0;
...
function downgradePredicateToFunction(type) {
    return {
        kind: 'function',
...
function addPredicateTestToConstraintList(node, tests, consequent, isTopLevelAndLastValStmt, env, constraints) {
    ...
    for (const test of tests) {
        const argVarType = lookupType(test.argVarName, env);
...
function _infer(node, env, constraints, isTopLevelAndLastValStmt = false) {
    ...
    switch (node.type) {
        case 'UnaryExpression': {
            ...
            const funcType = lookupType(op, env); // in either case its a monomorphic type
        ...
        case 'LogicalExpression': {
            ...
            try {
                ...
                if (e instanceof internalTypeErrors_1.UnifyError) {
                    addTypeError(new typeErrors_1.InvalidArgumentTypesError(node, [leftNode, rightNode], [tBool, tBool], [receivedLeftType, receivedRightType]));
                }
            }
            return newConstraints;
        }
        ...
        case 'ForStatement': {
            ...
            if (initNode.type === 'VariableDeclaration' &&
                ...
                setType(initName, initNode.declarations[0].init.inferredType, env);
                setDeclKind(initName, initNode.kind, env);
                newConstraints = infer(initNode, env, newConstraints);
                setType(initName, tForAll(applyConstraints(initNode.declarations[0].init
        ...
        case 'BlockStatement': {
            ...
            declNodes.forEach(declNode => {
                if (declNode.type === 'FunctionDeclaration' && declNode.id !== null) {
                    ...
                    setType(declName, declNode.declarations[0].init.inferredType, env);
            ...
            declNodes.forEach(declNode => {
                if (declNode.type === 'FunctionDeclaration' && declNode.id !== null) {
                    setType(declNode.id.name, tForAll(applyConstraints(declNode.functionInferredType, newConstraints)), env);
                }
                else if (declNode.type === 'VariableDeclaration' &&
                    declNode.declarations[0].id.type === 'Identifier') {
                    setType(declNode.declarations[0].id.name, tForAll(applyConstraints(declNode.declarations[0].init
                        .inferredType, newConstraints)), env);
                }
            });
        ...
        case 'ArrowFunctionExpression': {
            ...
            paramNodes.forEach((paramNode) => {
                setType(paramNode.name, paramNode.inferredType, env);
            });
            ...
            try {
                ...
                if (e instanceof internalTypeErrors_1.UnifyError) {
                    if (leftNode.type === 'Identifier') {
                        ...
                        addTypeError(new typeErrors_1.ArrayAssignmentError(node, tArray(applyConstraints(leftType, newConstraints)), tArray(applyConstraints(rightType, newConstraints))));
                    }
                }
            }
            return newConstraints;
        ...
        case 'ArrayExpression': {
            ...
            elements.forEach(element => {
                try {
                    ...
                    if (e instanceof internalTypeErrors_1.UnifyError) {
                        addTypeError(new typeErrors_1.ArrayAssignmentError(node, applyConstraints(node.inferredType, newConstraints), tArray(applyConstraints(element.inferredType, newConstraints))));
                    }
                }
            });
            return newConstraints;
...
const listFuncs = [['list', tForAll(tVar('T1'))]];
