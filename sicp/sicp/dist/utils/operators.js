...
function throwIfTimeout(nativeStorage, start, current, line, column) {
    if (current - start > nativeStorage.maxExecTime) {
        throw new timeoutErrors_1.PotentialInfiniteLoopError(create.locationDummyNode(line, column), nativeStorage.maxExecTime);
    }
}
...
function binaryOp(operator, chapter, left, right, line, column) {
    left = forceIt(left);
    right = forceIt(right);
    const error = rttc.checkBinaryExpression(create.locationDummyNode(line, column), operator, chapter, left, right);
    if (error === undefined) {
